...
<head>
  ...
  <script src="<%=path2%>/js/jquery-3.4.1.min.js" type="text/javascript"></script>
</head>
<body>
<%--搜索框--%>
...
<%--增加和修改表单start--%>
<form class="layui-form layui-form-panel" id="add-form" name="add-form" style="display: none;"
      lay-filter="first1">
...
<script>
  layui.use(['table', 'layer', 'form', 'laydate','upload'], function () {
    ...
    upload.render({
      ...
      ,filed: 'file'//表示上传的文件将会为名为 "headpic" 的字段发送到服务器。
      ,method: 'post'
      ,done: function(res){
        layer.alert(res.msg);
    ...
    table.on('tool(test)', function (obj) {
      ...
      if (layEvent === 'updateRole') {
        ...
        $.ajax({
          type: "post",
          url: "<%=path2%>/role/role_selectRole",
          data: "",
          async: false, // 同步
          success: function (datas) {
            //发起请求查询用户的角色 设置为false，则所有的请求均为同步请求，在没有返回值之前，同步请求将锁住浏览器，用户其它操作必须等待请求完成才可以执行。
            $.ajax({
              type: "post",
              url: "<%=path2%>/role/role_selectRoleIdByUserId",
              async: false, // 同步
    ...
    $("#savaRole").click(function () {
      $.ajax({
        type: "post",
        url: "<%=path2%>/role/role_updateUserRole",
    ...
    function openUpdateWindow(data) {
      ...
      mainindex = layer.open({
        ...
        content: $("#add-form"),//引用的窗口代码
        success: function () {
          form.val("first1", data);//lay-filter="first1"
          url = "update_user";
        }
      });
    }
